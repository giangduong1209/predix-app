...
interface ISetBetPositionProps {
  ...
  underOrOverStatus?: string;
  onEnterOverOrUnder?: (status: string) => void;
...
const SetDicePosition: React.FC<ISetBetPositionProps> = ({
  showSetBetCard,
  underOrOverStatus,
  onEnterOverOrUnder,
  onBackward,
  ...
  useEffect(() => {
    if (underOrOverStatus !== "") inputRef?.current?.focus();
  }, [inputRef, underOrOverStatus]);
  ...
  const changeOverOrUnderHandler = (status: string) => {
    if (onEnterOverOrUnder) return onEnterOverOrUnder(status);
  };
                  ...
                  <div className="flex-1 w-0 p-2">
                    <div className="flex items-start">
                      ...
                      <div className="ml-3 flex-1">
                        ...
                        <p className="mt-1 text-sm text-[--colors-text]">
                          {underOrOverStatus} position entered
                        </p>
                        <a
                          href={`https://testnet.bscscan.com/tx/${transaction.transactionHash}`}
                          className="mt-1 text-sm text-[--colors-primary]"
  ...
  return (
    ...
    >
      <div className="flex justify-between items-center bg-gradient-to-r from-[--colors-gradientCardHeaderFrom] to-[--colors-gradientCardHeaderTo] p-2 md:p-4 rounded-t-2xl">
      ...
      <div className="card-body p-4">
        ...
        <div className="px-4 md:py-2 bg-[--colors-input] rounded-2xl">
          <input
        ...
        <p className="hidden md:block text-[--colors-textSubtle] font-medium text-xs">
...
export default SetDicePosition;
